...
using Multiple_Test.Service.WindowsService;
...
{
    public partial class FrmHipotManual : UIForm
    {
        ...
        private void BtnOpenScript_Click(object sender, EventArgs e)
        {
            ...
            var script = file_servic.ConvertStepScript(SelectScriptPath);
            if (script.Count == 0)
            {
        ...
        {    //计算测试总时间
            ...
            while (true)
            {
                ...
                {   //清除上次测试结果
                    ...
                    //输入治具号
                    var checkVehicle = await service.InputVehicleNumberManual();
                    if (checkVehicle.Flag)
                    {

                        if (radioSnFlag.Checked)
                        {
                            var snlist = new pageInputSerialNumber(1);
                            snlist.ShowDialog();
                            ///获取扫码的条码
                            if (snlist.ScanInfo.Count != 1)
                            {
        ...
        private bool EquipmentTester()
        {
            ...
            //清除测试步骤
            if (service.ClearEquipmentStep(ref com, ref cardType, ref message))
            {
                SetComInfo(com, cardType);
                //配置測試步驟22
                ...
                default:
                    {
                        ...
                        //pageHipotFail fr = new pageHipotFail(txtVehicleNumber.Text);

                        //fr.ShowDialog();
                        //Cavity = fr.result;
                        //FlagMisjudge = fr.FlagMisjudge;
                        //FlagPending = fr.FlagPending;
        ...
        private void UploadMes(string Result_Final, int Cavity)
        {
            ...
            for (int i = 0; i < entity.Count; i++)
            {
                ...
                listdata.Add($"Step0{i + 1}-Fixture:NA");
                listdata.Add($"Step0{i + 1}-Vehicle:NA");
