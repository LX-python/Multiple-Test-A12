...
{
    public partial class FrmHipotSemi_Automatic : UIForm
    {
        ...
        private void FrmHipot_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.ShowAskDialog("您確定要退出半自动Hi-Pot系統嗎?"))
            {
                ClostFlag = true;
            }
            else
            ...
            while (true)
            {
                ...
                {   //清除上次测试结果
                    ...
                    if (checkVehicle.Flag)
                    {
                        ...
                        this.ShowSuccessDialog("请您手动压合治具后按确定按钮 进行测试");
                        this.ShowWaitForm("测试中...请等待...");
