...
{
    public partial class pageSTM32DownloadAutoScan7230 : UIForm
    {
        ...
        public pageSTM32DownloadAutoScan7230()
        {
            InitializeComponent();
            Initialize();
                ...
                if (File.Exists(Path.Combine(runPath, proConfig)))
                {
                    ...
                    if (config?.fwAutoDownload?.Pci_7230 != null)
                    {
                        ...
                        if (cardHandle < 0)
                        {
                            ...
                            ShowCommand("请检查设备安装是否正确，驱动程序是否安装，设备编号是否正确。" + message);
                        }
                        else
                        {
                            var DI_Down_Vlot = io_Map.GetMappingByPin(config.fwAutoDownload.Pci_7230.DI_Cylinder_Down);
                            ShowCommand($"气缸下降 DI-{DI_Down_Vlot.Value.IOIndex} Pin->{config.fwAutoDownload.Pci_7230.DI_Cylinder_Down}");
                            var DO_UP = io_Map.GetMappingByPin(config.fwAutoDownload.Pci_7230.DO_Cyclinder_up);
                            ShowCommand($"气缸上升 DO-{DO_UP.Value.IOIndex} Pin->{config.fwAutoDownload.Pci_7230.DO_Cyclinder_up}");
                            var DO_Vlot3 = io_Map.GetMappingByPin(config.fwAutoDownload.Pci_7230.DO_Voltage3);
                            ShowCommand($"3.3V DO-{DO_Vlot3.Value.IOIndex} Pin->{config.fwAutoDownload.Pci_7230.DO_Voltage3}");
                            MCUflag = true;
                            ledMcu.Color = System.Drawing.Color.Lime;
                            ledMcu.Blink = true;
                            ShowCommand($"PCI-7230 设备连接成功! 句柄: {cardHandle}");
            ...
            if (cardHandle >= 0)
            {
                try
                {
                    ...
                    if (result < 0)
                    {
                        ...
                        ShowCommand($"写入 {command} 失败！错误码: {result} ");
                    }
                    else
                    {

                        ShowCommand($"写入 {command} 成功！输出值: {result}");
                    }
