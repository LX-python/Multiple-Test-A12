...
namespace NXP_TEA_IC
{
    public partial class FormTest : Form
    {
        ...
        private SerialPort serialPortScan;
        ...
        private int DO_UP_Index = 0, DO_Voltage3_index = 0;
        private DIDOMap io_Map = new DIDOMap();
        //
        private bool DO_UP_Flag, DO_Voltage3_Flag;
        //新增用于MES上传的全局变量
        int n = 0;
        string vername = "";
        ...
        private void Initialize()
        {
            ...
            this.FormClosing += FormGUI_FormClosing;
            this.Load += FormGUI_Load;
        ...
        {
            listBoxLogs.Items.Add("控制器 PCI-7230 配置");
            var frm = new FrmPCI7230("AutoFWMCU", "控制器 PCI-7230 配置");
            frm.ShowDialog();
            LoadConfig();
        }
        ...
        {
            listBoxLogs.Items.Add("SR-710 扫码器配置");
        ...
        private async Task AutoRun()
        {
            ...
            {
                listBoxLogs.Items.Add("請配置MCU COM");
            ...
            {
                listBoxLogs.Items.Add("請配置Scan COM");
                ButtonsService.EnableBtn(btnRun);
                return;
            }
            ...
            if (string.IsNullOrEmpty(label_mifVersion.Text))
            {
                listBoxLogs.Items.Add("請選擇FW文件");
                ButtonsService.EnableBtn(btnRun);
                return;
            }
        ...
        private void FormGUI_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.ShowAskDialog("您確定要退出Firmware Download 系統嗎?"))
            {
                ...
                if (cardHandle >= 0)
                {
                    ...
                    listBoxLogs.Items.Add("PCI7230 设备已断开连接!");
                }
            }
            else
            {
        ...
        /// 转换Command
        /// </summary>
        /// <param name="DO_UP"></param>
        /// <param name="DO_Voltage3"></param>
        /// <returns></returns>
        private ushort ConventCommand(int DO_UP, int DO_Voltage3)
        {
            string binString = string.Empty;
            for (int i = 0; i < 16; i++)
            {
                string str = "0";
                if ((i == 15 - DO_UP && DO_UP_Flag) || (i == (15 - DO_Voltage3) && DO_Voltage3_Flag))
                {
                    str = "1";
            ...
            return Convert.ToUInt16(binString, 2);
        }
        private void SendMcuCommand(ushort command)
        {
            if (cardHandle >= 0)
            {
                try
                {
                    ...
                    // 调用 DLL 函数写入端口
                    short result = PCI7230.DO_WritePort(cardHandle, 0, command);
                    ...
                    if (result < 0)
                    {
                        ...
                        listBoxLogs.Items.Add($"写入 {command} 失败！错误码: {result} ");
                    }
                    else
                    {

                        listBoxLogs.Items.Add($"写入 {command} 成功！输出值: {result}");
                    }
                ...
                {
                    // 捕获异常并记录日志
                    listBoxLogs.Items.Add($"发生异常: {ex.Message}");
            ...
            else
            {
                ...
                this.ShowErrorNotifier("请连接PCI-7230 Card");
            }
        }
        ...
        private async void _serialPortScan_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            if (!SN_Status)
            {
                ...
                listBoxLogs.Items.Add($" Serial-Number:{receivedData}");
                txtSerialNumber.Text = receivedData;
                await MESCheckAsync(receivedData);
            }
            else
            {
                listBoxLogs.Items.Add("非正常模式触发扫码");
            }

        }

        private async Task Monitor_cylinder_downforce(int di_Volt)
        {
            ////Thread.Sleep(200);
            ...
            while (true)
            {
                try
                {
                    ...
                    {
                        ushort output;
                        PCI7230.DI_ReadPort(cardHandle, 0, out output);
                        var binString = output.ToBinString();
                        // 读取数字输入
                        // 遍历所有端口并读取数据
                        //
                        // var binString = "0000000000000001";
                        for (int i = 0; i < binString.Length; i++)
                        {
                            //ShowCommand($" DI 数据: {output.ToBinString()} ");

                            // 记录日志 0000 0000 0000 0001 / 0 16-1-0=0
                            if (i == (binString.Length - 1) - di_Volt)
                            {
                                var value = int.Parse(binString[i].ToString());
                                //  ShowCommand($" I={i} DI_Volt={di_Volt} 数据: {output.ToBinString()} ");
                                if (value == 1 && !scanFlag && !SN_Status)
                                {
                                    scanFlag = true;
                                ...
                                {
                                    listBoxLogs.Items.Add("等待治具下压 进入下一个流程");
                                    SN_Status = false;
                                    scanFlag = false;
                                }
                            }
                    ...
                    {
                        this.ShowErrorDialog("PCI7230没有连接");
                    }
        ...
        private bool OpenSerialPort()
        {
            ...
            try
            {
                ...
                if (File.Exists(Path.Combine(runPath, proConfig)))
                {
                    ...
                    //PCI-7230
                    if (config?.fwAutoDownload?.Pci_7230 != null)
                    {
                        ...
                        if (cardHandle < 0)
                        {
                            ...
                            listBoxLogs.Items.Add("请检查设备安装是否正确，驱动程序是否安装，设备编号是否正确。" + message);
                        }
                        else
                        {
                            var DI_Down_Vlot = io_Map.GetMappingByPin(config.fwAutoDownload.Pci_7230.DI_Cylinder_Down);
                            listBoxLogs.Items.Add($"气缸下降 DI-{DI_Down_Vlot.Value.IOIndex} Pin->{config.fwAutoDownload.Pci_7230.DI_Cylinder_Down}");
                            var DO_UP = io_Map.GetMappingByPin(config.fwAutoDownload.Pci_7230.DO_Cyclinder_up);
                            listBoxLogs.Items.Add($"气缸上升 DO-{DO_UP.Value.IOIndex} Pin->{config.fwAutoDownload.Pci_7230.DO_Cyclinder_up}");
                            var DO_Vlot3 = io_Map.GetMappingByPin(config.fwAutoDownload.Pci_7230.DO_Voltage3);
                            listBoxLogs.Items.Add($"3.3V DO-{DO_Vlot3.Value.IOIndex} Pin->{config.fwAutoDownload.Pci_7230.DO_Voltage3}");
                            MCUflag = true;
                            ledMcu.Color = System.Drawing.Color.Lime;
                            ledMcu.Blink = true;
                            listBoxLogs.Items.Add($"PCI-7230 设备连接成功! 句柄: {cardHandle}");
                            DO_UP_Index = DO_UP.Value.IOIndex;
                            DO_Voltage3_index = DO_Vlot3.Value.IOIndex;
                            //转换信息脚位命令
                            //初始化 DO 
                            listBoxLogs.Items.Add("初始化 DO ");
                            PCI7230.DO_WritePort(cardHandle, 0, 0b0000000000000000);
                            //启动气缸下压监控
                            listBoxLogs.Items.Add("启动气缸下压监控");
                            Monitor_cylinder_downforce(DI_Down_Vlot.Value.IOIndex);
                        }

                    }
                }

                return MCUflag && serialPortScan != null && serialPortScan.IsOpen;
            }
            catch (Exception ex)
            {
                this.ShowErrorDialog(ex.Message);
        ...
        private async Task<Task> MESCheckAsync(string serialNumber)
        {
            ...
            listBoxLogs.Items.Add($"检查 MES SerialNumber:{serialNumber} Status:{status}");
            if (!status)
            {
                ///给电3.3V
                mCU33V开ToolStripMenuItem.PerformClick();
        ...
        {
            DO_UP_Flag = true;
            DO_Voltage3_Flag = false;
            SendMcuCommand(ConventCommand(DO_UP_Index, DO_Voltage3_index));
            listBoxLogs.Items.Add("气缸上升");
        }
        ...
        private void 气缸可下降ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            DO_Voltage3_Flag = false;
            SendMcuCommand(ConventCommand(DO_UP_Index, DO_Voltage3_index));
            listBoxLogs.Items.Add("气缸可下降");
        }

        private void mCU33V开ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            DO_UP_Flag = false;
            DO_Voltage3_Flag = true;
            SendMcuCommand(ConventCommand(DO_UP_Index, DO_Voltage3_index));
            listBoxLogs.Items.Add("开启 3.3V ");
        }
        ...
        {
            string account = lab_UserName.Text;
            string res_message = string.Empty;
            listBoxLogs.Items.Add(vername);
       
                    button_searchTargetIC_Click(sender, e);
                    Thread.Sleep(2000);
                    //Task.Delay(2000).Wait();
                    buttonWriteDevice_Click(sender, e);
                    if ((label_Target_1_Version.Text == null) || (label_Target_1_Version.Text == ""))
                    {
                        label_Target_1_Red.Visible = true;
                        listBoxLogs.Items.Add("Target1 not find ic");
                        listBoxLogs.TopIndex = listBoxLogs.Items.Count - 1;
                    }
                    if ((label_Target_2_Version.Text == null) || (label_Target_2_Version.Text == ""))
                    {
                        label_Target_2_Red.Visible = true;
                        listBoxLogs.Items.Add("Target2 not find ic");
                        listBoxLogs.TopIndex = listBoxLogs.Items.Count - 1;
                    }
                    if ((label_Target_3_Version.Text == null) || (label_Target_3_Version.Text == ""))
                    {
                        label_Target_3_Red.Visible = true;
                        listBoxLogs.Items.Add("Target3 not find ic");
                        listBoxLogs.TopIndex = listBoxLogs.Items.Count - 1;
                    }
                    if ((label_Target_4_Version.Text == null) || (label_Target_4_Version.Text == ""))
                    {
                        label_Target_4_Red.Visible = true;
                        listBoxLogs.Items.Add("Target4 not find ic");
                        listBoxLogs.TopIndex = listBoxLogs.Items.Count - 1;
                    }

                    if (label_Target_1_Green.Visible == true)
                    {
                        MES_Service.SerialNumberCorssingStationVername(strSN, vername, ref res_message);
                        listBoxLogs.Items.Add(res_message);
                        listBoxLogs.TopIndex = listBoxLogs.Items.Count - 1;
                        MES_Service.SerialNumberCorssingStationPass(strSN, ref res_message);//command3 過站 +條碼+長度
                        listBoxLogs.Items.Add(res_message);
                        listBoxLogs.TopIndex = listBoxLogs.Items.Count - 1;

                    }
                    else if (label_Target_1_Red.Visible == true)
                    {
                        if (strSN != "")
                        {
                            MES_Service.SerialNumberCorssingStationFail(strSN, "ZX2", ref res_message);//command3 過站 +條碼+長度
                            listBoxLogs.Items.Add(res_message);
                            listBoxLogs.TopIndex = listBoxLogs.Items.Count - 1;
                        }
                    }
        ...
        private async Task<Task> DownloadFwHexAsync(string serialnumber)
        {
            ...
            {                 
                    string res_message = string.Empty;
                    listBoxLogs.Items.Add(vername);
                    button_searchTargetIC.PerformClick();
                    Thread.Sleep(1000);
                    buttonWriteDevice.PerformClick();
                    if ((label_Target_1_Version.Text == null) || (label_Target_1_Version.Text == ""))
                    {
                        label_Target_1_Red.Visible = true;
                        listBoxLogs.Items.Add("Target1 not find ic");
